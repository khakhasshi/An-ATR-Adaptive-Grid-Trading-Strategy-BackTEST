...
class AdaptiveGridTradingStrategy(bt.Strategy):
    params = (
        ('initial_grid_size', 0.1),  # 初始网格大小
        ('max_grid_orders', 30),  # 最大网格订单数
        ('atr_period', 17),  # ATR计算周期
    )
    ...
    def next(self):
        for data in self.datas:
            ...
            # 使用当前 ATR 动态调整网格大小
            grid_size = self.atr[data_name][0]
            self.grid_sizes.append(grid_size)  # 记录当前网格大小
    ...
    def plot_figures(self):
        ...
        # 绘制网格大小变化
        plt.subplot(2, 1, 2)
        plt.plot(self.grid_sizes, label='网格大小', color='blue')
        plt.title('网格大小随时间的变化')
        plt.xlabel('时间（单位：bars）')
        plt.ylabel('网格大小')
        plt.legend()
